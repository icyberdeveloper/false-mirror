...
class LostFilm:
    ...
    def build_config(self):
        ...
        cfg = {}
        cfg['lf_session'] = '5dd9ff9da1fdac450fd1bc10e77186d3.2214896'
        cfg['enabled'] = True
        cfg.setdefault('prefilter', False)
        cfg['site_urls'] = ["https://www.lostfilm.tv/"]
        return cfg

    def on_task_input(self):
        ...
        while site_urls:
            ...
            logger.debug('Trying to get and parse the RSS feed: {}', rss_url)
            try:
                ...
                if status == 200:
                    logger.debug('Received RSS feed from {}', rss_url)
                    break
                logger.info(
        ...
        for idx, item in enumerate(rss.entries, 1):
            ...
            if item.get('title') is None:
                ...
                logger.debug('Got RSS item title: {}', item['title'])
            ...
            if not series_name_org or season_num is None or episode_num is None:
                ...
                logger.debug('Using imprecise information from RSS item \'link\'')
            ...
            while site_urls:
                ...
                logger.debug('Trying to get the redirect page: {}', redirect_url)
                try:
            ...
            if not perfect_match:
                logger.debug('Trying to find series names in the final torrents download page')
    ...
    def _simplify_name(name: str) -> str:
        name = name #normalize_unicode(name)
            ...
            if isinstance(s_item, str):
                ...
                if s_cfg.get('alternate_name'):
                    if isinstance(s_cfg['alternate_name'], str):
                        ...
                        raise Exception(f'Cannot read series "alternate_name" for "{s_name:s}"')
            else:
                raise Exception(
